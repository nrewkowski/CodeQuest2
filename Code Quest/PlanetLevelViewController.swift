...
class PlanetLevelViewController: LevelViewController, UIPickerViewDelegate, UIPickerViewDataSource, UIGestureRecognizerDelegate, SSSpeechSynthesizerDelegate {
	...
	override func viewDidLoad() {
		...
		var queueFrame = UIImageView(frame: CGRect(x: 0, y: Int(Int(ButtonView.frame.minY-ButtonView.frame.height-50)), width: Int(self.view.frame.width), height: Int(ButtonView.frame.height)*2))
		var consoleview = UIImage(named: "consolefull")
		queueFrame.contentMode = .scaleAspectFill
		//consoleview.cont
		//queueFrame.backgroundColor = UIColor(patternImage: UIImage(named: "consolefull")!)
		queueFrame.image = UIImage(named: "consolefull")
		self.view.addSubview(queueFrame)
		self.view.sendSubview(toBack: queueFrame)
		ButtonView.initializeButtons()
		//adding the grid to the gamescene in scenekit
		ButtonView.gameControllerView = self
		ButtonView.backgroundColor = UIColor.clear
		//ButtonView.backgroundColor = UIColor(patternImage: UIImage(named: "consolebottom")!)
...
//	}
		...
		if (commandQueue.count > 0) {
			if (commandQueue[commandQueue.count-1] != 8){
				...
				*/
				...
				tempCell.frame = CGRect(x: LevelViewController.scaleDims(input: (84*commandQueue.count) % 880, x: true) + 40, y: LevelViewController.scaleDims(input: 480 + 90*(commandQueue.count/11), x: false), width: LevelViewController.scaleDims(input:80, x: true), height: LevelViewController.scaleDims(input: 80, x: false))
				...
				var loopLabel=UILabel(frame: CGRect(x: LevelViewController.scaleDims(input: (84*commandQueue.count) % 880, x: true) + 40, y: LevelViewController.scaleDims(input: 480 + 90*(commandQueue.count/11), x: false), width: LevelViewController.scaleDims(input:80, x: true), height: LevelViewController.scaleDims(input: 80, x: false)))
	...
	**/
	...
	override func getButtonInput(type:ButtonType) {
		...
		if (takeInput) {
			if (type.rawValue < 5 && commandQueue.count < 21) { // If command is to be added to queue and queue is not full
			...
			} else if(type.rawValue < 5 && commandQueue.count >= 21) { //the queue is full
